...
class admin(commands.Cog):
	...
	async def staffcommands(self, ctx):
		...
		print(current_time+" | "+guild.name+" | "+channel.name+" | "+user.name+"#"+user.discriminator,"Has checked staff commands.")
		
		em = discord.Embed(color=discord.Colour(0xffffff))
		em.set_author(name="Solyx staff commands!", icon_url=user.avatar_url)
		em.add_field(name="\n_ _\n**\u27a4** -blacklist".format(ctx.prefix), value="ban command for users, user cant play anymore.", inline=False)
		em.add_field(name="\n_ _\n**\u27a4** -give".format(ctx.prefix), value="give users items roles race class anything (- amount subtracts)", inline=False)
		em.add_field(name="\n_ _\n**\u27a4** -clear".format(ctx.prefix), value="clear certain amount of text.", inline=False)
		em.add_field(name="\n_ _\n**\u27a4** -dashboard".format(ctx.prefix), value="check bot / cog status", inline=False)
		em.add_field(name="\n_ _\n**\u27a4** -cogs".format(ctx.prefix), value="check cog status", inline=False)
		em.add_field(name="\n_ _\n**\u27a4** -load / unload / reload".format(ctx.prefix), value="load unload or reload cogs", inline=False)
		em.add_field(name="\n_ _\n**\u27a4** -py".format(ctx.prefix), value="run python code", inline=False)
		em.add_field(name="\n_ _\n**\u27a4** -shutdown".format(ctx.prefix), value="shut down the bot......", inline=False)
		try:
