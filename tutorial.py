...
#No response
	...
	@commands.group(name="tutorial", pass_context=True, no_pm=True, aliases=["tutorials"])
	...
	async def tutorial(self, ctx):
		...
		valid_options = ["-tutorial"]
		answer = await self.check_tutorial_answer(user, channel, valid_options, time)
		if answer:
			em = discord.Embed(title="Let's get started!", description="Type **{}Begin** to start your journey!".format(ctx.prefix), color=discord.Colour(0xffffff))
			...
			valid_options = ["-start", "-begin"]
			answer = await self.check_tutorial_answer(user, channel, valid_options, time)
			if answer:
			
				em = discord.Embed(title="LETS GO!", description="You just started your character!".format(answer, ctx.prefix), color=discord.Colour(0xffffff))
				#em.add_field(name="", value="", inline=False)
				em.set_author(name="Solyx Tutorial", icon_url=user.avatar_url)
				#em.set_footer(text="")
				print ('tutorial 2/10')
				...
				valid_options = ["orc", "human", "elf", "demon", "-orc", "-human", "-elf", "-demon"]
				answer = await self.check_tutorial_answer(user, channel, valid_options, time)
				if answer:
					await asyncio.sleep(2)
					em = discord.Embed(title="Good choice!", description="The {} race is a lot of fun to play!\nLet's choose your race!".format(answer, ctx.prefix), color=discord.Colour(0xffffff))
					#em.add_field(name="", value="", inline=False)
					em.set_author(name="Solyx Tutorial", icon_url=user.avatar_url)
					#em.set_footer(text="")
					print ('tutorial 3/10')
					...
					valid_options = ["Archer", "Knight", "Mage", "Thief", "archer", "knight", "mage", "thief", "-Archer", "-Knight", "-Mage", "-Thief", "-archer", "-knight", "-mage", "-thief"]
					answer = await self.check_tutorial_answer(user, channel, valid_options, time)
					if answer:
						await asyncio.sleep(2)
						...
						answer = await self.check_tutorial_answer(user, channel, valid_options)
						if answer:
							await asyncio.sleep(2)
